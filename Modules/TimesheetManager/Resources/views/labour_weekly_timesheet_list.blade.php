@php
    ...
    $sunday_total_tmj = 0;
@endphp
<table class="table-responsive table timesheet-table" id="timesheet-table">
    <tr>
...
<table class="table-responsive table table-bordered">				
	<thead class="table-primary">
        <tr class="text-center">
            ...
            <th><span class="vfont">RATE</span></th>
        ...
        @if($timesheets->isNotEmpty())   
            @foreach($timesheets as $timesheet)
                @php
					...
					$supervisor = ucfirst($user->full_name ?? '');
                ...
                $timesheets1 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $monday)->where('operative', $supervisor_name)->get();
                        
          
                    if($timesheets1->isNotEmpty()){
                        foreach($timesheets1 as $timesheet1){
                            $per_total_cost = $per_estimate * (float)$timesheet1->hours;	
                            $tt = (float)$timesheet1->hours;	 
                            $a = '';
                            $a = (float)$timesheet1->hours;
                            if (strpos($a, ':') !== false) {
                    ...
                    $timesheets2 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $tuesday)->where('operative', $supervisor_name)->get();
                    
                    
                    if($timesheets2->isNotEmpty()){
                        foreach($timesheets2 as $timesheet2){
                            $per_total_cost = $per_estimate * (float)$timesheet2->hours;	
	 
                            $tt = (float)$timesheet2->hours;	 
                            $a = '';
                            $a = (float)$timesheet2->hours;
                            if (strpos($a, ':') !== false) {
                    ...
                    $timesheets3 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $wednesday)->where('operative', $supervisor_name)->get();
                    
                   
                    if($timesheets3->isNotEmpty()){
                        foreach($timesheets3 as $timesheet3){
                            $per_total_cost = $per_estimate * (float)$timesheet3->hours;	
	 
                            $tt = (float)$timesheet3->hours;	 
                            $a = '';
                            $a = (float)$timesheet3->hours;
                            if (strpos($a, ':') !== false) {
                    ...
                    $timesheets4 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $thursday)->where('operative', $supervisor_name)->get();
                    
                    if($timesheets4->isNotEmpty()){
                        foreach($timesheets4 as $timesheet4){
                            $per_total_cost = $per_estimate * (float)$timesheet4->hours;	
	 
                            $tt = (float)$timesheet4->hours;	 
                            $a = '';
                            $a = (float)$timesheet4->hours;
                            if (strpos($a, ':') !== false) {
                    ...
                    $timesheets5 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $friday)->where('operative', $supervisor_name)->get();
                     
                    if($timesheets5->isNotEmpty()){
                        foreach($timesheets5 as $timesheet5){
                            $per_total_cost=$per_estimate * (float)$timesheet5->hours;	
	 
                            $tt = (float)$timesheet5->hours;	 
                            $a = '';
                            $a = (float)$timesheet5->hours;
                            if (strpos($a, ':') !== false) {
                    ...
                    $timesheets6 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $saturday)->where('operative', $supervisor_name)->get();
                   
                    if($timesheets6->isNotEmpty()){
                        foreach($timesheets6 as $timesheet6){
                            
                            $per_total_cost = $per_estimate * (float)$timesheet6->hours;	
	 
                            $tt = (float)$timesheet6->hours;	 
                            $a = '';
                            $a = (float)$timesheet6->hours;
                            if (strpos($a, ':') !== false) {
                    ...
                    $timesheets7 = Modules\TimesheetManager\Entities\LabourTimesheet::join('labour_timesheet_materials', 'labour_timesheet_materials.labour_timesheet_id', '=', 'labour_timesheets.id')->where('activity_id', $timesheet->activity_id)->whereDate('timesheet_date', $sunday)->where('operative', $supervisor_name)->get();
                    
                    
                    if($timesheets7->isNotEmpty()){
                        foreach($timesheets7 as $timesheet7){
                            $per_total_cost=$per_estimate * (float)$timesheet7->hours;	
	 
                            $tt= (float)$timesheet7->hours;	 
                            $a = '';
                            $a = (float)$timesheet7->hours;
                            if (strpos($a, ':') !== false) {
                ...
                <tr align="center">	
                    ...
                    <td>{{$rate}}</td>
                    <td>
                        <input type="checkbox" id="timesheet_approval{{ $timesheet->id }}" name="timesheet_approval[]" onchange="approveTimesheets('{{ $timesheet->id }}', '{{ $timesheet->project_id }}', '{{ $timesheet->activity_id }}', '{{ $monday }}', '{{ $sunday }}', '{{ $supervisor_name }}');"  value="{{ auth()->id() }}" @if($timesheet->approver_id > 0) checked @endif>
                    </td>
                    <td></td>
                </tr>
            @endforeach
            ...
            <tr align="center">
                ...
                <td>{{($week_total_hours.'.'.$week_total_hours_tmj)*$rate}}</td>
                <td></td>
                <td></td>
            </tr>
        @endif
